...
- 🛡️ **Стелс-маскировка и Anti-RKN шум (Decoy Camouflage):**
  - Панель скрыта на случайном высоком порту (20000–60000) и в секретной директории (URL-пути, например, `/node-8f2a` или кастомном).
  - Стандартный путь `/login` заблокирован для сканеров и посторонних посетителей.
  - На корне `/` развернут сайт-приманка (Decoy) с маскировкой под веб-сервер Nginx (`Server: nginx/1.24.0`), облачный узел телеметрии, API документацию или черную дыру 404, исключающий обнаружение Hysteria и панели автоматическими сканерами РКН и провайдеров.
